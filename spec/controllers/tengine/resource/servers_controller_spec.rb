require 'spec_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to specify the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator.  If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails.  There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.
#
# Compared to earlier versions of this generator, there is very limited use of
# stubs and message expectations in this spec.  Stubs are only used when there
# is no simpler way to get a handle on the object needed for the example.
# Message expectations are only used when there is no simpler way to specify
# that an instance is receiving a specific message.

describe Tengine::Resource::ServersController do

  # This should return the minimal set of attributes required to create a valid
  # Tengine::Resource::Server. As you add validations to Tengine::Resource::Server, be sure to
  # update the return value of this method accordingly.
  def valid_attributes
    {}
  end

  describe "GET index" do
    it "assigns all tengine_resource_servers as @tengine_resource_servers" do
      Tengine::Resource::Server.delete_all
      server = Tengine::Resource::Server.create! valid_attributes
      get :index
      servers = assigns(:servers)
      servers.to_a.should eq([server])
    end
  end

  describe "GET show" do
    it "assigns the requested server as @server" do
      server = Tengine::Resource::Server.create! valid_attributes
      get :show, :id => server.id.to_s
      assigns(:server).should eq(server)
    end
  end

  describe "GET new" do
    it "assigns a new server as @server" do
      get :new
      assigns(:server).should be_a_new(Tengine::Resource::Server)
    end
  end

  describe "GET edit" do
    it "assigns the requested server as @server" do
      server = Tengine::Resource::Server.create! valid_attributes
      get :edit, :id => server.id.to_s
      assigns(:server).should eq(server)
    end
  end

  describe "POST create" do
    describe "with valid params" do
      it "creates a new Tengine::Resource::Server" do
        expect {
          post :create, :server => valid_attributes
        }.to change(Tengine::Resource::Server, :count).by(1)
      end

      it "assigns a newly created server as @server" do
        post :create, :server => valid_attributes
        assigns(:server).should be_a(Tengine::Resource::Server)
        assigns(:server).should be_persisted
      end

      it "redirects to the created server" do
        post :create, :server => valid_attributes
        response.should redirect_to(Tengine::Resource::Server.last)
      end
    end

    describe "with invalid params" do
      it "assigns a newly created but unsaved server as @server" do
        # Trigger the behavior that occurs when invalid params are submitted
        Tengine::Resource::Server.any_instance.stub(:save).and_return(false)
        post :create, :server => {}
        assigns(:server).should be_a_new(Tengine::Resource::Server)
      end

      it "re-renders the 'new' template" do
        # Trigger the behavior that occurs when invalid params are submitted
        Tengine::Resource::Server.any_instance.stub(:save).and_return(false)
        post :create, :server => {}
        response.should render_template("new")
      end
    end
  end

  describe "PUT update" do
    describe "with valid params" do
      it "updates the requested server" do
        server = Tengine::Resource::Server.create! valid_attributes
        # Assuming there are no other tengine_resource_servers in the database, this
        # specifies that the Tengine::Resource::Server created on the previous line
        # receives the :update_attributes message with whatever params are
        # submitted in the request.
        Tengine::Resource::Server.any_instance.should_receive(:update_attributes).with({'these' => 'params'})
        put :update, :id => server.id, :server => {'these' => 'params'}
      end

      it "assigns the requested server as @server" do
        server = Tengine::Resource::Server.create! valid_attributes
        put :update, :id => server.id, :server => valid_attributes
        assigns(:server).should eq(server)
      end

      it "redirects to the server" do
        server = Tengine::Resource::Server.create! valid_attributes
        put :update, :id => server.id, :server => valid_attributes
        response.should redirect_to(server)
      end
    end

    describe "with invalid params" do
      it "assigns the server as @server" do
        server = Tengine::Resource::Server.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Tengine::Resource::Server.any_instance.stub(:save).and_return(false)
        put :update, :id => server.id.to_s, :server => {}
        assigns(:server).should eq(server)
      end

      it "re-renders the 'edit' template" do
        server = Tengine::Resource::Server.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Tengine::Resource::Server.any_instance.stub(:save).and_return(false)
        put :update, :id => server.id.to_s, :server => {}
        response.should render_template("edit")
      end
    end
  end

  describe "DELETE destroy" do
    it "destroys the requested server" do
      server = Tengine::Resource::Server.create! valid_attributes
      expect {
        delete :destroy, :id => server.id.to_s
      }.to change(Tengine::Resource::Server, :count).by(-1)
    end

    it "redirects to the tengine_resource_servers list" do
      server = Tengine::Resource::Server.create! valid_attributes
      delete :destroy, :id => server.id.to_s
      response.should redirect_to(tengine_resource_servers_url)
    end
  end

end
