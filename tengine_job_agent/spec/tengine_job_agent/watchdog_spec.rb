...
describe TengineJobAgent::Watchdog do
  ...
  describe "#process" do
    ...
    it "spawnする" do
      ...
      subject.stub(:wait_process).with(pid).and_return(stat)
    ...
    it "子プロセスを待つ" do
      ...
      subject.should_receive(:wait_process).and_return(stat)
      subject.stub(:fire_finished).with(pid, stat)
      sender = mock(:sender)
      sender.stub_chain(:mq_suite, :ensures).with(:connection).and_yield
      subject.stub(:sender).and_return(sender)
    ...
    context "ファイルへの出力" do
      ...
      it "実行に成功した場合はPIDが出力される" do
        ...
        subject.should_receive(:wait_process).with(pid)
        EM.run do
          EM.add_timer(0.1) { EM.stop }
          subject.process
        end
