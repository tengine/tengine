...
# error.jobnet.job.tengine(2) #
  シナリオ: [異常系]error.jobnet.job.tengineのイベント処理中に、error.jobnet.job.tengineを発火した後に、tenginedがダウンする_error.jobnet.job.tengineがイベント処理失敗イベントの前に処理される
    ...
    かつ Coreサーバを落とすテストのためのドライバに置き換えるために"Coreサーバ1, Coreサーバ2"、"echo `bundle exec gem which tengine_job`|sed -e 's/\(.*\)tengine_job.rb/\1/'|xargs -i \cp -rf tengine_console/features/usecases/アプリケーション運用者がジョブを実行中にCoreサーバがダウンしたことを確認してフ ェイルバックする/driver/error.jobnet.job.tengine/1/1/drivers {}tengine/job/drivers"コマンドを実行する
