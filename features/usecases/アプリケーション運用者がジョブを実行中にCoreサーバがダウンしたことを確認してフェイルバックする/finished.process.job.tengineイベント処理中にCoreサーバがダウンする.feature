...
# (1)
  シナリオ: [異常系]finished.process.core.tengineのイベント処理中に、ジョブストアのジョブネットの状態を更新した後に、Coreサーバがダウンする_スクリプトが正常終了していた
    ...
    かつ Coreサーバを落とすテストのためのドライバに置き換えるために"Coreサーバ1, Coreサーバ2"、"echo `bundle exec gem which tengine_job`|sed -e 's/\(.*\)tengine_job.rb/\1/'|xargs -i \cp -rf tengine_console/features/usecases/アプリケーション運用者がジョブを実行中にtenginedがダウンしたことを確認してフェイルバックする/driver/finished.process.job.tengine/1/1/drivers {}tengine/job/drivers"コマンドを実行する
