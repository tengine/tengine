...
# start.jobnet.job.tengine(1) #
  シナリオ: [異常系]start.jobnet.job.tengineのイベント処理中に、ジョブストアのジョブネットの状態を更新した後に、tenginedがダウンする
    ...
    かつ Coreサーバを落とすテストのためのドライバに置き換えるために"Coreサーバ1, Coreサーバ2"、"echo `bundle exec gem which tengine_job`|sed -e 's/\(.*\)tengine_job.rb/\1/'|xargs -i \cp -rf tengine_console/features/usecases/job/アプリケーション運用者がジョブを実行中にCoreサーバがダウンしたことを確認してフ ェイルバックする/driver/start.jobnet.job.tengine/1/1/drivers {}tengine/job/drivers"コマンドを実行する
    ...
    もし "実行ジョブ一覧画面"を表示する
    かつ "jn0004"の"ステータス変更"リンクをクリックする
    ならば "ジョブステータス変更"画面を表示していること
...
# 以下フェイルバックの確認
  ...
  シナリオ: [異常系]start.jobnet.job.tengineのイベント処理中に、start.jobnet.job.tengineやstart.job.job.tengineを全て発火した後に、tenginedがダウンする_start.jobnet.job.tengineがイベント処理失敗イベントの前に処理される
    ...
    もし Coreサーバを落とすために"Coreサーバ2"で"\cp -f tengine_console/feature/config/emergency_test/start.execution.job.tengine_1_yml tengine_console/config/emergency_test.yml"コマンドを実行する
    もし "Coreサーバ2"上で"Tengineコアプロセス2"の起動を行うために"tengined -T tengine_console/usecases/job/dsl/1055_2_layer_in_jobnet_boot_jobs_is_jobnet.rb -f ./features/config/tengined.yml.erb "というコマンドを実行する
    ならば "Tengineコアプロセス2"の状態が"稼働中"であることを確認できること
    ...
    かつ 以下の行が表示されていること
    |ジョブネット名|説明       |操作     |
    |jobnet1055  |jobnet1055|閲覧 実行|

    もし "テンプレートジョブ一覧画面"を表示する
    かつ "jobnet1055"の"実行"リンクをクリックする
    ならば "ジョブネット実行設定画面"を表示していること
    ...
    かつ 以下の行が表示されていること
    |ジョブ名         |ステータス|操作         |
    |jobnet1055      |準備中    |表示 再実行  |
    |  job1          |初期化済  |表示 強制停止|

    もし 10秒間待機する
    ならば 以下の行が表示されていること
    |ジョブ名         |ステータス|操作         |
    |jobnet1055      |正常終了    |表示 再実行  |
    |  job1          |正常終了    |表示 強制停止|
    ...
    ならば 以下の行が表示されていること
    |ジョブネット名|ステータス|操作       |
    |jobnet1055      |状態不明  |監視 ステータス変更 再実行|

    もし "jobnet1055"の"ステータス変更"リンクをクリックする
    ならば "ジョブステータス変更"画面を表示していること
