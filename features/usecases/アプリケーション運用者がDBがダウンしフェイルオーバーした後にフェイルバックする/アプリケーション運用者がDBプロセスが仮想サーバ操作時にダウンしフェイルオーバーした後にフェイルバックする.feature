...
# --------------------------
  ...
  シナリオ: 仮想マシンの起動前にMongoDBのプライマリーのプロセスがダウンした際にフェイルオーバーし、その後アプリケーション運用者がDBプロセスをフェールバックする
   ...
   もし mongod_sがPRIMARYになっていることを確認するために"ssh root@#{mongod_s_ip} command \"mongo features/step_definitions/mongodb/status.js"コマンドを実行する
   ならば mongod_pの"stateStr"が"(not reachable/healthy)"と表示されていること
   ならば mongod_sのいずれかの"stateStr"が"PRIMARY"と表示されていること
  ...
  シナリオ: [異常系]仮想マシンの停止中にMongoDBのプライマリーのプロセスがダウンした際にフェイルオーバーし、その後アプリケーション運用者がDBプロセスをフェールバックする
    ...
    # ステータスがrunningになるまで時間がかかります(環境によって異なります)
    |物理サーバ名             |仮想サーバ名|プロバイダによるID  |説明|IPアドレス|ステータス|仮想サーバイメージ名|仮想サーバタイプ|
    |physical_server_name_01|run_1_virtual_servers001|virtual_server_uuid_01|仮想サーバを1台起動テストの説明|private_ip_address: 192.168.1.1|terminated|virtual_server_image_uuid_01|virtual_server_spec_uuid_01|
