#language:ja
機能: アプリケーション運用者がhadoop_job_runをタイムアウト強制停止する


  ジョブの実行時間が正常な動作ではあり得ないほど長い場合
  アプリケーション運用者
  はhadoop_job_runを自動的に強制停止したい
  背景:
    前提 日本語でアクセスする
  ...
  シナリオ: [正常系]fork前、ルートジョブネットの最初
    ...
    もし "テンプレートジョブ一覧画面"を表示する
    かつ "jn0004_hadoopの"実行"リンクをクリックする
  ...
  シナリオ: [正常系]実行中のジョブが1つで片方がエラー終了している状態でタイムアウト強制停止
    ...
    かつ 以下の行が表示されていること
    |ID|ジョブ名  |説明     |実行スクリプト               |接続サーバ名|認証情報名      |開始日時           |終了日時|ステータス|次のジョブ|操作       |
    |  |j1        |j1       |$HOME/0004_retry_one_layer_hadoop.sh|test_server1|test_credential1|2011/11/25 14:43:22|        |実行中    |j2, j3    |表示 再実行|
    |  |  hadoop_job1|      |                                    |test_server1|test_credential1|                   |        |実行中    |          |表示   |
    |  |    Map   |         |                                    |test_server1|test_credential1|                   |        |実行中    |          |表示   |
    |  |    Reduce|         |                                    |test_server1|test_credential1|                   |        |実行中    |          |表示   |
    |  |j2        |j2       |$HOME/0004_retry_one_layer_hadoop.sh|test_server1|test_credential1|                   |        |初期化済  |j4        |表示 再実行  |
...
#--retry2--
  ...
  シナリオ: [正常系]ルートジョブネット内のジョブネット内のジョブ
    ...
    かつ 以下の行が表示されていること
    |ID|ジョブ名        |説明       |実行スクリプト               |接続サーバ名|認証情報名      |開始日時           |終了日時|ステータス|次のジョブ|操作         |
    |  |j1              |j1         |$HOME/0005_retry_two_layer_hadoop.sh|test_server1|test_credential1|2011/11/25 14:43:22|        |準備中    |j2, jn4   |表示 強制停止|
    |  |  hadoop_job1   |           |                                    |test_server1|test_credential1|                   |        |準備中    |          |表示   |
    |  |    Map         |           |                                    |test_server1|test_credential1|                   |        |準備中    |          |表示   |
    |  |    Reduce      |           |                                    |test_server1|test_credential1|                   |        |準備中    |          |表示   |
    |  |j2              |j2         |$HOME/0005_retry_two_layer_hadoop.sh|test_server1|test_credential1|                   |        |初期化済  |j4        |表示 再実行  |
    |  |  hadoop_job2   |           |                                    |test_server1|test_credential1|                   |        |初期化済  |          |表示   |
    |  |    Map         |           |                                    |test_server1|test_credential1|                   |        |初期化済  |          |表示   |
    |  |    Reduce      |           |                                    |test_server1|test_credential1|                   |        |初期化済  |          |表示   |
    |  |jn4             |jn4        |                                    |test_server1|test_credential1|                   |        |初期化済  |j4        |表示 再実行  |
    |  |  j41           |j41        |$HOME/0005_retry_two_layer_hadoop.sh|test_server1|test_credential1|                   |        |初期化済  |j42,j43   |表示 再実行  |
    |  |    hadoop_job41|           |                                    |test_server1|test_credential1|                   |        |初期化済  |          |表示   |
    |  |      Map       |           |                                    |test_server1|test_credential1|                   |        |初期化済  |          |表示   |
    |  |      Reduce    |           |                                    |test_server1|test_credential1|                   |        |初期化済  |          |表示   |
  ...
  シナリオ: [正常系]ジョブネットの中のジョブネット内にあるジョブを実行中にタイムアウト強制停止
    ...
    |  |    J111    |j111   |$HOME/0006_retry_three_layer_hadoop.sh|test_server1|test_credential1|                   |        |準備中    |j112      |表示 強制停止|
    |  |      hadoop_job111||                                      |test_server1|test_credential1|                   |        |準備中    |          |表示   |
    |  |        Map        ||                                      |test_server1|test_credential1|                   |        |準備中    |          |表示   |
    |  |        Reduce     ||                                      |test_server1|test_credential1|                   |        |準備中    |          |表示   |
...
#retry1
  ...
  シナリオ: [正常系]ジョブの状態が「開始中」で強制停止を行う
    前提 "Tengineコアプロセス"がオプション" -f ./features/config/tengine_debug.yml -T usecases/job/dsl/0004_retry_one_layer_hadoop.rb --process-daemon"で起動している
    ...
    # Execution
    もし 実行ジョブ"jn0004_hadoopのExecutionを"execution"と呼ぶことにする
    # /jobnet1001 => jn0004_hadoop    かつ 実行ジョブ"jn0004_hadoopのルートジョブネット"/jn0004_hadoopを"root_jobnet"と呼ぶことにする
    # next!start@/jn0004_hadoop=> e1
    かつ 実行ジョブ"jn0004_hadoopのエッジ"next!start@/jn0004_hadoopを"e1"と呼ぶことにする
    # /jn0004_hadoopj1 => j1
    もし 実行ジョブ"jn0004_hadoopのジョブ"/jn0004_hadoopj1"を"j1"と呼ぶことにする
    # next!/jn0004_hadoopj1 => e2
    かつ 実行ジョブ"jn0004_hadoopのエッジ"next!/jn0004_hadoopj1"を"e2"と呼ぶことにする

    # prev!/jn0004_hadoopj2 => e3
    かつ 実行ジョブ"jn0004_hadoopのエッジ"prev!/jn0004_hadoopj2"を"e3"と呼ぶことにする
    # /jn0004_hadoopj2 => j2
    もし 実行ジョブ"jn0004_hadoopのジョブ"/jn0004_hadoopj2"を"j2"と呼ぶことにする
    # next!/jn0004_hadoopj2 => e4
    かつ 実行ジョブ"jn0004_hadoopのエッジ"next!/jn0004_hadoopj2"を"e4"と呼ぶことにする

    # prev!/jn0004_hadoopj3 => e5
    かつ 実行ジョブ"jn0004_hadoopのエッジ"prev!/jn0004_hadoopj3"を"e5"と呼ぶことにする
    # /jn0004_hadoopj3 => j3
    もし 実行ジョブ"jn0004_hadoopのジョブ"/jn0004_hadoopj3"を"j3"と呼ぶことにする
    # next!/jn0004_hadoopj3 => e6
    かつ 実行ジョブ"jn0004_hadoopのエッジ"next!/jn0004_hadoopj3"を"e6"と呼ぶことにする

    # prev!/jn0004_hadoopj4 => e7
    かつ 実行ジョブ"jn0004_hadoopのエッジ"prev!/jn0004_hadoopj4"を"e7"と呼ぶことにする
    # /jn0004_hadoopj4 => j4
    もし 実行ジョブ"jn0004_hadoopのジョブ"/jn0004_hadoopj4"を"j4"と呼ぶことにする
    # next!/jn0004_hadoopj4 => e8
    かつ 実行ジョブ"jn0004_hadoopのエッジ"next!/jn0004_hadoopj4"を"e8"と呼ぶことにする

    # /jn0004_hadoopfinally => finally
    もし 実行ジョブ"jn0004_hadoopのジョブ"/jn0004_hadoopfinally"を"finally"と呼ぶことにする
    # prev!/jn0004_hadoopfinally/jn0004_f => e9
    かつ 実行ジョブ"jn0004_hadoopのエッジ"prev!/jn0004_hadoopfinally/jn0004_f"を"e9"と呼ぶことにする
    # /jn0004_hadoopfinally/jn0004_f => jn0004_hadoop    もし 実行ジョブ"jn0004_hadoopのジョブ"/jn0004_hadoopfinally/jn0004_f"を"jn0004_f"と呼ぶことにする
    # next!/jn0004_hadoopfinally/jn0004_f => e10
    かつ 実行ジョブ"jn0004_hadoopのエッジ"next!/jn0004_hadoopfinally/jn0004_f"を"e10"と呼ぶことにする
    ...
    ならば 以下の行が表示されていること
    |ID|ジョブネット名|説明  |開始日時|終了日時|ステータス|操作       |
    |  |jn0004_hadoop       |jn0004_hadoop|        |        |強制停止済|監視 再実行|


#--retry2
  @05_02_01_14
  シナリオ: [正常系]ルートジョブネット内のジョブネット内のジョブ
    前提 "Tengineコアプロセス"がオプション" -f ./features/config/tengine_debug.yml -T usecases/job/dsl/0005_retry_two_layer_hadoop.rb --process-daemon"で起動している
    ...
    # /jobnet1001 => jn0005_hadoop
    かつ 実行ジョブ"jn0005_hadoop"のルートジョブネット"/jn0005_hadoop"を"root_jobnet"と呼ぶことにする
    # next!start@/jn0005_hadoop => e1
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"next!start@/jn0005_hadoop"を"e1"と呼ぶことにする
    # /jn0005_hadoop/j1 => j1
    もし 実行ジョブ"jn0005_hadoop"のジョブ"/jn0005_hadoop/j1"を"j1"と呼ぶことにする
    # next!/jn0005_hadoop/j1 => e2
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"next!/jn0005_hadoop/j1"を"e2"と呼ぶことにする

    # prev!/jn0005_hadoop/j2 => e3
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"prev!/jn0005_hadoop/j2"を"e3"と呼ぶことにする
    # /jn0005_hadoop/j2 => j2
    もし 実行ジョブ"jn0005_hadoop"のジョブ"/jn0005_hadoop/j2"を"j2"と呼ぶことにする
    # next!/jn0005_hadoop/j2 => e4
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"next!/jn0005_hadoop/j2"を"e4"と呼ぶことにする

    # prev!/jn0005_hadoop/jn4 => e5
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"prev!/jn0005_hadoop/jn4"を"e5"と呼ぶことにする
    # /jn0005_hadoop/jn4 => jn4
    もし 実行ジョブ"jn0005_hadoop"のジョブ"/jn0005_hadoop/jn4"を"jn4"と呼ぶことにする
    # next!/jn0005_hadoop/jn4 => e6
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"next!/jn0005_hadoop/jn4"を"e6"と呼ぶことにする

    # next!start@/jn0005_hadoop/jn4 => e7
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"next!start@/jn0005_hadoop/jn4"を"e7"と呼ぶことにする
     # /jn0005_hadoop/jn4/j41 => j41
    もし 実行ジョブ"jn0005_hadoop"のジョブ"/jn0005_hadoop/jn41"を"jn41"と呼ぶことにする
    # next!/jn0005_hadoop/jn4/j41 => e8
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"next!/jn0005_hadoop/jn4/j41"を"e8"と呼ぶことにする

    # prev!/jn0005_hadoop/jn4/j42 => e9
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"prev!/jn0005_hadoop/jn4/j42"を"e9"と呼ぶことにする
    # /jn0005_hadoop/jn4/j42 => j42
    もし 実行ジョブ"jn0005_hadoop"のジョブ"/jn0005_hadoop/jn4/j42"を"j42"と呼ぶことにする
    # next!/jn0005_hadoop/jn4/j42 => e10
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"next!/jn0005_hadoop/jn4/j42"を"e10"と呼ぶことにする

    # prev!/jn0005_hadoop/jn4/j43 => e11
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"prev!/jn0005_hadoop/jn4/j43"を"e11"と呼ぶことにする
    # /jn0005_hadoop/jn4/j43 => j43
    もし 実行ジョブ"jn0005_hadoop"のジョブ"/jn0005_hadoop/jn4/j43"を"j43"と呼ぶことにする
    # next!/jn0005_hadoop/jn4/j43 => e12
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"next!/jn0005_hadoop/jn4/j43"を"e12"と呼ぶことにする

    # prev!/jn0005_hadoop/jn4/j44 => e13
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"prev!/jn0005_hadoop/jn4/j44"を"e13"と呼ぶことにする
    # /jn0005_hadoop/jn4/j44 => j44
    もし 実行ジョブ"jn0005_hadoop"のジョブ"/jn0005_hadoop/jn4/j44"を"j44"と呼ぶことにする
    # next!/jn0005_hadoop/jn4/j44 => e14
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"next!/jn0005_hadoop/jn4/j44"を"e14"と呼ぶことにする

    # /jn0005_hadoop/jn4/finally => jn4/finally
    もし 実行ジョブ"jn0005_hadoop"のジョブ"/jn0005_hadoop/jn4/finally"を"jn4/finally"と呼ぶことにする
    # "next!start@/jn0005_hadoop/jn4/finally" => e15
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"next!start@/jn0005_hadoop/jn4/finally"を"e15"と呼ぶことにする
    # /jn0005_hadoop/jn4/finally/jn4_f => jn4_f
    もし 実行ジョブ"jn0005_hadoop"のジョブ"/jn0005_hadoop/jn4/finally/jn4_f"を"jn4_f"と呼ぶことにする
    # next!/jn0005_hadoop/jn4/finally/jn4_f => e16
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"next!/jn0005_hadoop/jn4/finally/jn4_f"を"e16"と呼ぶことにする

    # prev!/jn0005_hadoop/j4 => e17
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"prev!/jn0005_hadoop/j4"を"e17"と呼ぶことにする
    # /jn0005_hadoop/j4 => j4
    もし 実行ジョブ"jn0005_hadoop"のジョブ"/jn0005_hadoop/j4"を"j4"と呼ぶことにする
    # next!/jn0005_hadoop/j4 => e18
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"next!/jn0005_hadoop/j4"を"e18"と呼ぶことにする

    # /jn0005_hadoop/finally => finally
    もし 実行ジョブ"jn0005_hadoop"のジョブ"/jn0005_hadoop/finally"を"finally"と呼ぶことにする
    # next!start@/jn0005_hadoop/finally => e19
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"next!start@/jn0005_hadoop/finally"を"e19"と呼ぶことにする
    # /jn0005_hadoop/finally/jn0005_fjn => jn0005_fjn
    もし 実行ジョブ"jn0005_hadoop"のジョブ"/jn0005_hadoop/finally/jn0005_fjn"を"jn0005_fjn"と呼ぶことにする
    # next!/jn0005_hadoop/finally/jn0005_fjn => e20
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"next!/jn0005_hadoop/finally/jn0005_fjn"を"e20"と呼ぶことにする

    # next!start@/jn0005_hadoop/finally/jn0005_fjn => e21
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"next!start@/jn0005_hadoop/finally/jn0005_fjn"を"e21"と呼ぶことにする
    # /jn0005_hadoop/finally/jn0005_fjn/jn0005_f1 => jn0005_f1
    もし 実行ジョブ"jn0005_hadoop"のジョブ"/jn0005_hadoop/finally/jn0005_fjn/jn0005_f1"を"jn0005_f1"と呼ぶことにする
    # next!/jn0005_hadoop/finally/jn0005_fjn/jn0005_f1 => e22
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"next!/jn0005_hadoop/finally/jn0005_fjn/jn0005_f1"を"e22"と呼ぶことにする
    # /jn0005_hadoop/finally/jn0005_fjn/jn0005_f2 => jn0005_f2
    もし 実行ジョブ"jn0005_hadoop"のジョブ"/jn0005_hadoop/finally/jn0005_fjn/jn0005_f2"を"jn0005_f2"と呼ぶことにする
    # next!/jn0005_hadoop/finally/jn0005_fjn/jn0005_f2 => e23
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"next!/jn0005_hadoop/finally/jn0005_fjn/jn0005_f2"を"e23"と呼ぶことにする


    # /jn0005_hadoop/finally/jn0005_fjn/finally => jn0005_fjn/finally
    もし 実行ジョブ"jn0005_hadoop"のジョブ"/jn0005_hadoop/finally/jn0005_fjn/finally"を"jn0005_fjn/finally"と呼ぶことにする
    # next!start@/jn0005_hadoop/finally/jn0005_fjn => e24
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"next!start@/jn0005_hadoop/finally/jn0005_fjn"を"e24"と呼ぶことにする
    # /jn0005_hadoop/finally/jn0005_fjn/finally/jn0005_fif => jn0005_fif
    もし 実行ジョブ"jn0005_hadoop"のジョブ"/jn0005_hadoop/finally/jn0005_fjn/finally/jn0005_fif"を"jn0005_fif"と呼ぶことにする
    # next!/jn0005_hadoop/finally/jn0005_fjn/finally/jn0005_fif => e25
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"next!/jn0005_hadoop/finally/jn0005_fjn/finally/jn0005_fif"を"e25"と呼ぶことにする

    # /jn0005_hadoop/finally/jn0005_f => jn0005_f
    もし 実行ジョブ"jn0005_hadoop"のジョブ"/jn0005_hadoop/finally/jn0005_f"を"jn0005_f"と呼ぶことにする
    # next!/jn0005_hadoop/finally/jn0005_f => e26
    かつ 実行ジョブ"jn0005_hadoop"のエッジ"next!/jn0005_hadoop/finally/jn0005_f"を"e26"と呼ぶことにする
    ...
    ならば 以下の行が表示されていること
    |ID|ジョブネット名|説明  |開始日時|終了日時|ステータス|操作       |
    |  |jn0005_hadoop        |jn0005_hadoop |        |        |強制停止済|監視 再実行|
