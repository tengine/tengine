...
#
  ...
  シナリオ: [異常系]アプリケーション開発者がTengineコアのイベントハンドラ定義を作成・実行する_イベントドライバ一覧画面でイベントドライバを有効から無効に変更できない_他のアプリケーション開発者が誤ってイベントドライバを削除
    #
    ...
    # 異常を発生させるためrails runner を使用してイベントドライバを削除する
    もし "イベントドライバの削除"を行うために"rails runner 'Tengine::Driver.delete_all'"というコマンドを実行する

    # 存在しないイベントドライバに更新を行おうとするので、RecordNotFoundエラーになる
    もし "無効ボタン"をクリックする
    ならば "イベントドライバ一覧画面"を表示されていないこと
    かつ "ActiveRecord::RecordNotFound"と表示されていること
...
# 
  ...
  シナリオ: [正常系]アプリケーション開発者がTengineコアのイベントハンドラ定義を作成・実行する_イベント通知画面で通知レベルがエラーのイベントのアラートを出さない_他のアプリケーション開発者がイベントを通知確認済みに変更
  # 最新状態を表示した際に通知レベルがerror,fatalで通知確認済みでないイベントはアラートを出す。他のアプリケーション開発者が対象のイベントを通知確認済みに変更した場合、アラートが検出されない