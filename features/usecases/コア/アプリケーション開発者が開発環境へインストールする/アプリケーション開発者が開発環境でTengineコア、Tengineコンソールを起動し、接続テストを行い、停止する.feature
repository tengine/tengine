...
#      かつ イベントキューを削除する
#      ならば "接続テスト"の標準出力に"event queue is not found."と出力されていること 
#
#      もし イベントキューを登録する
#
#      もし "接続テスト"を行うために"tengined -k test -f tengine.yml"というコマンドを実行する
...
#      # TODO 発火の直前で削除する必要がある
#      かつ イベントエクスチェンジを削除する
#      ならば "接続テスト"の標準出力に"event exchange is not found."と出力されていること 
#
#      もし イベントエクスチェンジを登録する
#      もし イベントエクスチェンジとイベントキューをバインドする
#
...
#      ならば サーバ構成レポートが作成されていること
  ...
  シナリオ: [異常系]Tengineコアのプロセスの起動に失敗し、問題を取り除いた後インストールを続行する_設定ファイルに誤り
      # yamlとしてフォーマットが不正なファイル
      前提 不正なTengineコアプロセスの設定ファイル"invalid_tengine.yml"が存在すること 
...
#      ならば サーバ構成レポートが作成されていること
  ...
  シナリオ: [異常系]Tengineコアのプロセスの停止に失敗し、強制停止を行う
      前提 処理が終了しないイベントハンドラ定義ファイル"./feature/event_handler_def/endless.rb"が存在すること

      もし "接続テスト"を行うために"tengined -k test -f tengine.yml"というコマンドを実行する
      ならば "接続テスト"の標準出力に"Success!"と出力されていること

      もし "Tengineコアプロセスの起動"を行うために"tengined -k start -f tengine.yml -T ./feature/event_handler_def/endless.rb"というコマンドを実行する
      ならば "Tengineコアプロセス"の標準出力からPIDを確認することができること
      ならば "Tengineコアプロセス"が起動していることをPIDを用いて"ps -eo pid PID"というコマンドで確認できること
      ...
      もし "処理が終了しないイベントハンドラとバインドされている"イベントを発火する
			
      かつ "Tengineコアプロセス"を Ctl+c で停止する
      ならば "Tengineコンソールプロセス"が起動していることをPIDを用いて"ps -eo pid PID"というコマンドで確認できること
