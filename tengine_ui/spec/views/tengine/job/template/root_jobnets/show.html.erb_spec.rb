...
describe "tengine/job/template/root_jobnets/show.html.erb" do
  before(:all) do
  ...
  describe "構成ジョブがないとき" do
    ...
    it "一覧画面へのリンクが表示されていること" do
      ...
      rendered.should have_link(name, :href => tengine_job_template_root_jobnets_path)
    end
  end
...
__end_of_dsl__
      ...
      @jobnet_templates = []
      visitor = \
        Tengine::Job::Structure::Visitor::All.new do |vertex|
    ...
    it "構成ジョブが表示されていること" do
      ...
      @jobnet_templates.each do |item|
        ...
        next unless destination.instance_of?(Tengine::Job::Template::Jobnet)
