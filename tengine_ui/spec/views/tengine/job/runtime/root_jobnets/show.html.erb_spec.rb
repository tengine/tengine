...
describe "tengine/job/runtime/root_jobnets/show.html.erb" do
  before(:each) do
  ...
  it "タイトルが表示されていること" do
    render(:file => "tengine/job/runtime/root_jobnets/show")

    rendered.should have_xpath("//h1",
      :text => I18n.t("tengine.job.runtime.root_jobnets.show.title"))
  end
  ...
  describe "@jobnet_actualsが空でないとき" do
    before do
      @job1 = stub_model(Tengine::Job::Runtime::SshJob,
      ...
      @job2 = stub_model(Tengine::Job::Runtime::SshJob,
    ...
    it "「一覧に戻る」リンクが表示されていること" do
      ...
      rendered.should have_link(I18n.t("views.links.back_list"),
        :href => tengine_job_runtime_root_jobnets_path)
    end
    ...
    it "全体の強制停止のリンクが表示されないこと" do
      ...
      rendered.should_not have_link(I18n.t("views.links.force_exit"),
        :href => tengine_job_runtime_root_jobnet_path(@root_jobnet_actual))
    end
    ...
    context "@job1のステータスがinitializedのとき" do
      ...
      it "再実行のリンクが表示されていること" do
        ...
        #   :href => edit_tengine_job_runtime_root_jobnet_jobnet_actual_path(
        #     @job1, :root_jobnet_actual_id => @root_jobnet_actual.id.to_s))
        rendered.should have_link(I18n.t("views.links.rerun"),
          :href => new_tengine_job_runtime_execution_path(
        ...
        pending
        rendered.should_not have_link(I18n.t("views.links.force_exit"),
          :href => tengine_job_runtime_root_jobnet_jobnet_actual_path(@job1.id.to_s,
    ...
    context "@jobnet1のステータスがstuck, @job2のステータスがrunningのとき" do
      before do
        @jobnet1 = stub_model(Tengine::Job::Runtime::SshJob,
          :id => Moped::BSON::ObjectId("4e955633c3406b3a9f000002"),
          :name => "job1 name",
          :description => "job1 description",
          :server_name => "job1 server_name",
      ...
      it "@jobnet1はステータス変更と再実行のリンクが表示されていること" do
        ...
        rendered.should_not have_link(I18n.t("views.links.force_exit"),
          :href => tengine_job_runtime_root_jobnet_jobnet_actual_path(@jobnet1.id.to_s,
            :root_jobnet_actual_id => @root_jobnet_actual.id.to_s))
      end
      ...
      it "@job2はステータス変更と強制停止のリンクが表示されていること" do
        ...
        rendered.should have_link(I18n.t("views.links.force_exit"),
          :href => tengine_job_runtime_root_jobnet_jobnet_actual_path(@job2.id.to_s,
            :root_jobnet_actual_id => @root_jobnet_actual.id.to_s))
      end
    end
    ...
    context "@root_jobnet_actualのステータスがrunningのとき" do
      ...
      it "メッセージ一覧のリンクが表示されていること" do
        ...
        rendered.should have_link(
          I18n.t("tengine.job.runtime.root_jobnets.show.links.events"),
    ...
    context "<BUG>hadoop_job_runを含むジョブを実行すると、「ジョブネット監視」画面のhadoop_job、Map、Reduceに強制停止リンク、及び再実行リンクが表示されてしまう" do
      [:hadoop_job, :map_phase, :reduce_phase].each do |jobnet_type_key|
        context "jobnet_type_keyが#{jobnet_type_key.inspect}ならば" do
          Tengine::Job::Runtime::SshJob.phase_keys.each do |phase_key|
            context "どんな状態でも(#{phase_key.inspect})" do
              before do
                @job1 = stub_model(Tengine::Job::Runtime::Jobnet,
                  :id => Moped::BSON::ObjectId("4e955633c3406b3a9f000002"),
