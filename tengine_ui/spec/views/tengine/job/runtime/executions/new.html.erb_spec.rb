...
describe "tengine/job/runtime/executions/new.html.erb" do
# for i18n lazy lookup
def render
  super(:file => "tengine/job/runtime/executions/new")
end
  ...
  describe "テンプレートジョブネットの実行のとき" do
    ...
    it "実行のタイトルが表示されていること" do
      ...
      rendered.should have_xpath("//h1",
        :text => I18n.t("tengine.job.runtime.executions.new.title"))
    end
    ...
    it "テンプレート参照画面へのリンクが表示されていること" do
      ...
      rendered.should have_link(@test.id.to_s,
        :href => tengine_job_template_root_jobnet_path(@test.id.to_s))
    end
  ...
  describe "実行ジョブネットの再実行のとき" do
    ...
    describe "ルートジョブネットの最初から再実行のとき" do
      ...
      it "再実行のタイトルが表示されていること" do
        ...
        rendered.should have_xpath("//h1",
          :text => I18n.t("tengine.job.runtime.executions.new.retry_title"))
      end
      ...
      it "テンプレート参照画面へのリンクが表示されていること" do
        ...
        rendered.should have_link(@test.template_id.to_s,
          :href => tengine_job_template_root_jobnet_path(@test.template_id.to_s))
      end
