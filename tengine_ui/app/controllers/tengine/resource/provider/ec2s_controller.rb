class Tengine::Resource::Provider::Ec2sController < ApplicationController
  ...
  def index
    @ec2s = Tengine::Resource::Provider::Ec2.all(:sort => [[:_id]]).page(params[:page])
  ...
  def show
    @ec2 = Tengine::Resource::Provider::Ec2.find(params[:id])
  ...
  def new
    @ec2 = Tengine::Resource::Provider::Ec2.new
  ...
  def create
    @ec2 = Tengine::Resource::Provider::Ec2.new(params[:ec2])

    respond_to do |format|
      if @ec2.save
        format.html { redirect_to @ec2, notice: successfully_created(@ec2) }
  ...
  def update
    ...
    respond_to do |format|
      if @ec2.update_attributes(params[:ec2])
        format.html { redirect_to @ec2, notice: successfully_updated(@ec2) }
        format.json { head :ok }
      else
