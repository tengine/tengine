...
jobnet("jn0006", :instance_name => "test_server1", :credential_name => "test_credential1") do
  ...
  jobnet("jn1", :to => "jn2") do
   ...
   jobnet("jn11", :to => "j12") do
     boot_jobs("j111")
     hadoop_job_run("j111", "$HOME/0006_retry_three_layer.sh",:to => "j112") do
       hadoop_job("hadoop_job111")
     end
     hadoop_job_run("j112", "$HOME/0006_retry_three_layer.sh" ) do
   ...
   hadoop_job_run("j12", "$HOME/0006_retry_three_layer.sh") do
     hadoop_job("hadoop_job12")
   end
   finally do
     hadoop_job_run("jn1_f","$HOME/0006_retry_three_layer.sh") do
  ...
  jobnet("jn2") do
   boot_jobs("j21")
   hadoop_job_run("j21", "$HOME/0006_retry_three_layer.sh", :to => "jn22") do
     hadoop_job("hadoop_job21")
   end
   jobnet("jn22") do
     boot_jobs("j221")
     hadoop_job_run("j221", "$HOME/0006_retry_three_layer.sh",:to => "j222") do
       hadoop_job("hadoop_job221")
     end
     hadoop_job_run("j222", "$HOME/0006_retry_three_layer.sh" ) do
       hadoop_job("hadoop_job222")
     end
     finally do
       hadoop_job_run("jn22_f","$HOME/0006_retry_three_layer.sh") do
         hadoop_job("hadoop_job22_f")
       end
     end
   end
   finally do
     hadoop_job_run("jn2_f","$HOME/0006_retry_three_layer.sh") do
